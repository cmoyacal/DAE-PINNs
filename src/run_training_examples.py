...
from utils.plots import plot_num_train_analysis
...
def main(args):
    ...
    print("usign device number...", torch.cuda.current_device())
    print("using...", device)

    # list of number of training examples
    num_train = [50, 100, 500, 1000, 2000]
    train_error = np.empty((len(num_train),))
    test_error = np.empty((len(num_train),))
    ...
    for k in range(len(num_train)):
        print("number of training examples...", num_train[k])
        X_train = geom.random_points(num_train[k])
        ...
        train_error[k] = state.best_loss_train
        test_error[k] = state.best_loss_test

    # plot results
    print("plotting results...\n")
    plot_num_train_analysis(num_train, train_error, test_error, fname=os.path.join(args.log_dir,'num-train-analysis.png'))

    # saving the results for post-processing
    np.savez(os.path.join(args.log_dir, "num-train"), num_train=num_train, train=train_error, test=test_error)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log-dir', type=str, default="./logs/dae-pinns-num-train-analysis-boole/", help="log dir")
